...
export function UserName2() {
  ...
  export async function confirmFaces(
    images: any,
    croppedFaces: Face[]
  ) {
  ...
  export function DisplayFace(props: {
    ...
    return (
      <div>
        ...
        <Checkbox label="Select" checked={isSelected} onChange={(ev:any,checked?:boolean)=>{
          if(checked!==undefined){
            ...
            if(checked){
              if(!isExists){
                ...
                tempSelected=tempSelected.filter(id=>id!==props.face.PersonId)
            }
          ...
          <Card.Body>
            ...
            <Card.Text>Criminal:{props.face.IsCriminal ? "Yes" : "No"}</Card.Text>
            
            </div>}
          </Card.Body>
          {props.isClass?null:<Button variant="primary" onClick={()=>{
            setEdit(!edit)
